<template>
    <div>
       ...
       <div class="p-3 flex items-center justify-center w-full h-screen align-middle dark:bg-gray-600">
        ...
        <form @submit.prevent="updatePassword"  class="space-y-4">
            ...
            required>
            <button class="py-1.5 px-5 bg-sky-500 rounded-md text-white hover:bg-sky-400 font-semibold">update</button>
        </form>
            </div>
            </div>
        </div>
...
async function updatePassword(){
    const {data,error} = await user.auth.updateUser({
